    ...
    oe_enclave_t* enclave_b)
{
    ...
    char* root = strstr(report, BEGIN_CERTIFICATE);
    char* last = NULL;
    while (root)
    {
        last = root;
        root = strstr(root + sizeof(BEGIN_CERTIFICATE), BEGIN_CERTIFICATE);
    }

    result = set_trusted_root(enclave_a, &ret, last, report_size - (last - report));
    if (result != OE_OK || ret)
    {
        printf("Host: set_trusted_root failed with %d/%d\n", result, ret);
        ret = 1;
        goto exit;
    }

    uint8_t attestation_report[4096];
    size_t attestation_report_size = sizeof(attestation_report);
    ret = remote_attest(
        report, report_size, attestation_report, &attestation_report_size);
    if (ret)
    {
        printf("Host: Remote attestation failed.\n");
        goto exit;
    }

    printf("Host: verify_report_and_store_certificate in %s\n", enclave_a_name);
    result = verify_report_and_store_certificate(
        enclave_a, &ret, attestation_report, report_size);
...
#define TA_UUID "97d140f4-5f59-4d1f-9735-cb21d49e7eb5"
...
int main(int argc, const char* argv[])
{
    ...
    {
        printf(
            "Host: generate_signed_message failed. %s", oe_result_str(result));
        if (ret == 0)
            ret = 1;
        goto exit;
    }
