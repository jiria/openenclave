...
int set_trusted_root(
    uint8_t* rot_cert_pem,
    size_t rot_cert_pem_size)
{
    oe_result_t result = OE_OK;
    mbedtls_x509_crt rot_cert = {0};

    mbedtls_x509_crt_init(&rot_cert);
    int ret =
        mbedtls_x509_crt_parse(&rot_cert, rot_cert_pem, rot_cert_pem_size);
    if (ret != 0)
    {
        result = OE_FAILURE;
        goto exit;
    ...
    result = set_remote_attestation_trusted_root(&rot_cert_pem, 1);
    if (result != OE_OK)
    {
        goto exit;
    }

    printf("set_trusted_root succeeded.\n");

exit:
    mbedtls_x509_crt_free(&rot_cert);
    ...
    size_t report_size)
{
    ...
    oe_result_t result = oe_verify_report(remote_report, report_size, &parsed_report);
    if (result != OE_OK)
    {
        goto exit;
    }

    if (!(parsed_report.identity.attributes & OE_REPORT_ATTRIBUTES_REMOTE))
    {
        result = OE_FAILURE;
        goto exit;
    }
    ...
    int ret = mbedtls_x509_crt_parse(&other_enclave_cert, remote_report, report_size);
    if (ret != 0)
    {
        result = OE_FAILURE;
        goto exit;
